        ...
        <div className="p-4 sm:p-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 gap-6 mt-8">
                ...
                <div className="bg-gray-800 rounded-2xl p-6">
                    ...
                    {deliveryRequests.length > 0 ? ( <div className="space-y-8"> {deliveryRequests.map(req => ( <div key={req.id} className="bg-gray-900/50 p-4 rounded-lg"> <div className="flex justify-between items-baseline mb-4"> <p className="font-bold">Demande du {formatDate(req.createdAt)}</p> <p className="text-sm text-gray-400">{req.items.length} article(s)</p> </div> <DeliveryStatusTracker status={req.status} reason={req.cancellationReason} /> </div> ))} </div> ) : <p className="text-gray-400">Aucune demande de livraison en cours.</p>}
                </div>
            </div>
