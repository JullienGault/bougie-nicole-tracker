...
import { 
    ...
    createUserWithEmailAndPassword, // Gardé pour la création par l'admin
    signOut 
} from 'firebase/auth';
import { 
    getFirestore, 
...
// ... Le reste des composants comme KpiCard, SaleModal, DeliveryRequestModal restent identiques

// =================================================================
// TABLEAU DE BORD ADMIN (AVEC CRÉATION D'UTILISATEUR)
// =================================================================
const CreatePosModal = ({ db, showToast, onClose }) => {
    ...
    const handleCreateUser = async (e) => {
        ...
        // Crée une instance d'application Firebase secondaire et temporaire pour créer un utilisateur sans déconnecter l'admin
        ...
        try {
            ...
            await setDoc(doc(db, "users", newUser.uid), {
                ...
                email: email,
                role: "pos", // Attribue le rôle de dépôt-vente
            ...
            setIsLoading(false);
            // Nettoyer la session de l'app secondaire
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-40 animate-fade-in" onClick={onClose}>
            <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-700 animate-fade-in-up" onClick={e => e.stopPropagation()}>
                <h2 className="text-2xl font-bold text-white mb-6">Ajouter un nouveau Dépôt-Vente</h2>
                <form onSubmit={handleCreateUser} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Nom du Dépôt-Vente</label>
                        <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} required className="w-full bg-gray-700 p-3 rounded-lg" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Email de connexion</label>
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full bg-gray-700 p-3 rounded-lg" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Mot de passe initial</label>
                        <input type="password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full bg-gray-700 p-3 rounded-lg" />
                    </div>
...
const AdminDashboard = ({ db, user, showToast }) => {
    ...
    // ... le reste du code de l'AdminDashboard (gestion des livraisons, etc.) est ici ...
    ...
    return (
        <div className="p-4 sm:p-8 animate-fade-in">
            ...
            {/* Le code pour les demandes de livraison reste ici... */}
            ...
            if (authUser) {
                ...
                const unsubscribeUser = onSnapshot(userDocRef, (doc) => {
                    if (doc.exists()) {
                        ...
                        setLoginError("Ce compte n'est pas configuré dans la base de données. Contactez l'administrateur.");
                        signOut(auth);
                    }
                    setIsLoading(false);
                }, (error) => {
             ...
             <div className="bg-gray-900 text-white min-h-screen font-sans">
                ...
                <main>
                    {userData.role === 'admin' ? 
                        ...
                        <div className="p-8 text-center"><h2 className="text-2xl">Bienvenue, Dépôt-Vente!</h2><p>Le tableau de bord POS sera affiché ici.</p></div>
                    }
                </main>
            </div>
        );
