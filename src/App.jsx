...
import {
    ...
    ArrowRightCircle, ImageUp, Loader2, Image as ImageIcon, Edit, ArrowLeft, Search, Layers
} from 'lucide-react';
...
const DeliveryRequestModal = ({ posId, posName, onClose }) => {
    ...
    const handleSend = async () => {
        ...
        if (validItems.length === 0) {
            showToast("Veuillez ajouter au moins un article avec une quantité valide.", "error");
            return;
        }
...
const ProductEditModal = ({ product, onClose }) => {
    ...
    const handleSave = async (e) => {
        ...
        if (!product && !imageFile) {
            showToast("Veuillez ajouter une image pour le nouveau produit.", "error");
            return;
        }
        setIsLoading(true);
        try {
                ...
                <form onSubmit={handleSave} className="space-y-4">
                     <div className="flex items-center gap-6">
                        ...
                        <div className="flex-1">
                            ...
                            <input id="image-upload" type="file" accept="image/*" onChange={handleFileChange} className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"/>
                        </div>
                     </div>
        ...
        <div className="p-4 sm:p-8 animate-fade-in">
            ...
            {isLoading ? (<div className="flex justify-center items-center p-16"><Loader2 className="animate-spin h-12 w-12"/></div>) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                    {filteredProducts.map(p => (<div key={p.id} className="bg-gray-800 rounded-2xl overflow-hidden shadow-lg flex flex-col group"><div className="h-48 bg-gray-700 flex items-center justify-center relative">{p.imageUrl ? (<img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" />) : (<ImageIcon className="text-gray-500" size={48} />)}<span className="absolute top-2 right-2 bg-indigo-500/80 text-white text-xs font-bold px-2 py-1 rounded-full">{p.category}</span></div><div className="p-4 flex flex-col flex-grow"><h4 className="font-bold text-lg text-white truncate" title={p.name}>{p.name}</h4><p className="text-indigo-400 font-semibold text-xl mt-1">{formatPrice(p.price)}</p><div className="mt-auto pt-4 flex justify-end gap-2"><button onClick={() => setProductToDelete(p)} className="p-2 rounded-lg bg-red-900/50 text-red-400 hover:bg-red-900/80"><Trash2 size={18} /></button><button onClick={() => openEditModal(p)} className="p-2 rounded-lg bg-blue-900/50 text-blue-400 hover:bg-blue-900/80"><Edit size={18} /></button></div></div></div>))}
                </div>
            )}
        </div>
    );
...
const PosDashboard = ({ pos, isAdminView = false, onActionSuccess = () => {} }) => {
    ...
    const kpis = useMemo(() => { const totalStock = stock.reduce((acc, item) => acc + item.quantity, 0); const totalRevenue = unsettledSales.reduce((acc, sale) => acc + sale.totalAmount, 0); const commission = totalRevenue * (posData?.commissionRate || 0); const netToBePaid = totalRevenue - commission; return { totalStock, totalRevenue, netToBePaid }; }, [stock, unsettledSales, posData]);

    const salesStats = useMemo(() => { if (salesHistory.length === 0) return []; const productSales = salesHistory.reduce((acc, sale) => { const key = sale.productName; acc[key] = (acc[key] || 0) + sale.quantity; return acc; }, {}); return Object.entries(productSales).sort(([,a],[,b]) => b-a).slice(0, 3); }, [salesHistory]);
    ...
    const handleDeleteSale = async (reason) => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                const stockDoc = await transaction.get(stockDocRef);
                if (!stockDoc.exists()) throw new Error("Stock introuvable pour la restauration.");
                
                const newQuantity = stockDoc.data().quantity + sale.quantity;
                transaction.update(stockDocRef, { quantity: newQuantity });
                transaction.delete(saleDocRef);
                transaction.set(logDocRef, { type: 'SALE_CANCELLED', reason, saleData: sale, cancelledAt: serverTimestamp(), by: currentUserData.email });
            });
