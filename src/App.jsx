...
const APP_TITLE = "Bougie Nicole Tracker"; // Titre pour l'onglet du navigateur
...
const DeliveryRequestModal = ({ db, posId, posName, onClose, showToast }) => {
    ...
    const handleItemChange = (index, field, value) => {
        ...
        if (field === 'productId') newItems[index].scent = '';
        setItems(newItems);
    };
    ...
    return (
         <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-40 animate-fade-in" onClick={onClose}>
            <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-2xl border border-gray-700 animate-fade-in-up custom-scrollbar max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={handleAddItem} className="mt-4 flex items-center gap-2 text-indigo-400 hover:text-indigo-300"><PlusCircle size={20} /> Ajouter un autre article</button>
                <div className="mt-8 flex justify-end gap-4">
                     <button onClick={onClose} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">Annuler</button>
...
const PosDashboard = ({ db, user, showToast }) => {
    ...
    return (
        <div className="p-4 sm:p-8">
            ...
            {saleToDelete && <ConfirmationModal title="Confirmer la suppression" message={`Supprimer la vente de ${saleToDelete.quantity} x ${saleToDelete.productName} ${saleToDelete.scent || ''}? Le stock sera restauré.`} onConfirm={handleDeleteSale} onCancel={() => setSaleToDelete(null)} requiresReason={true}/>}
            ...
            <div className="bg-gray-800 rounded-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setShowHistory(!showHistory)} className="flex items-center gap-2 text-indigo-400 hover:text-indigo-300">{showHistory ? 'Masquer l\'historique' : 'Voir l\'historique des ventes'} {showHistory ? <ChevronUp/> : <ChevronDown/>}</button>
                </div>
                
                {showHistory ? (
                    <div className="animate-fade-in overflow-x-auto">
                        <table className="w-full text-left">
                            <thead><tr className="border-b border-gray-700 text-gray-400 text-sm"><th className="p-3">Date</th><th className="p-3">Produit</th><th className="p-3">Quantité</th><th className="p-3">Total</th><th className="p-3">Actions</th></tr></thead>
                            <tbody>{salesHistory.map(sale => (<tr key={sale.id} className="border-b border-gray-700 hover:bg-gray-700/50"><td className="p-3">{formatDate(sale.createdAt)}</td><td className="p-3">{sale.productName} <span className="text-gray-400">{sale.scent}</span></td><td className="p-3">{sale.quantity}</td><td className="p-3 font-semibold">{formatPrice(sale.totalAmount)}</td><td className="p-3"><button onClick={() => setSaleToDelete(sale)} className="text-red-500 hover:text-red-400 p-1"><Trash2 size={18}/></button></td></tr>))}</tbody>
                        </table>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead><tr className="border-b border-gray-700 text-gray-400 text-sm"><th className="p-3">Produit</th><th className="p-3">Parfum</th><th className="p-3">Quantité en Stock</th><th className="p-3">Prix Unitaire</th></tr></thead>
                            <tbody>{stock.map(item => (<tr key={item.id} className="border-b border-gray-700 hover:bg-gray-700/50"><td className="p-3 font-medium">{item.productName}</td><td className="p-3 text-gray-300">{item.scent || 'N/A'}</td><td className={`p-3 font-bold ${item.quantity <= LOW_STOCK_THRESHOLD ? 'text-yellow-400' : 'text-white'}`}>{item.quantity}</td><td className="p-3">{formatPrice(item.price)}</td></tr>))}</tbody>
                        </table>
                    </div>
                )}
            </div>
...
const AdminDashboard = ({ db, user, showToast }) => {
    ...
    const [selectedPos, setSelectedPos] = useState(null);
     ...
     useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => setPointsOfSale(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))), console.error);
        return unsubscribe;
    }, [db]);
    ...
    const handleFulfillRequest = async (request) => {
         try {
            ...
            for (const item of request.items) {
                 ...
                 batch.set(stockDocRef, {productId: item.productId, productName: PRODUCTS.find(p => p.id === item.productId).name, price: PRODUCTS.find(p => p.id === item.productId).price, scent: item.scent || null, quantity: currentQuantity + Number(item.quantity)}, { merge: true });
            }
    ...
    if (selectedPos) {
        return (
            <div>
                 <button onClick={() => setSelectedPos(null)} className="m-4 bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">&larr; Retour à la liste</button>
                         ...
                         <div key={req.id} className="bg-gray-700/50 p-4 rounded-lg">
                             <div className="flex justify-between items-start">
                                 <div>
                                     ...
                                     <ul className="list-disc list-inside mt-2 text-sm">{req.items.map((item, idx) => (<li key={idx}>{item.quantity} x {PRODUCTS.find(p=>p.id === item.productId).name} {item.scent || ''}</li>))}</ul>
                                 </div>
                 ...
                 <div className="space-y-3">
                     {pointsOfSale.map(pos => (
                         <div key={pos.id} className="bg-gray-700/50 p-4 rounded-lg flex justify-between items-center">
                             ...
                             <button onClick={() => setSelectedPos(pos)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-3 rounded-lg flex items-center gap-2 text-sm"><Eye size={18}/> Voir le détail</button>
                         </div>
                     ))}
                 </div>
            </div>
