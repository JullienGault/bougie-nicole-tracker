...
const PosDashboard = ({ isAdminView = false, pos, onActionSuccess = () => {} }) => {
    ...
    const handleCreatePayout = async (reconciledData) => {
        ...
        try {
            await batch.commit();
            showToast("Période de paiement clôturée !", "success"); onActionSuccess();
    ...
    return (
        <div className="p-4 sm:p-8 animate-fade-in">
            {showReconciliationModal && posData && <PayoutReconciliationModal pos={posData} unsettledSales={unsettledSales} stock={stock} onClose={() => setShowReconciliationModal(false)} onConfirm={handleCreatePayout} isReadOnly={!isAdminView} />}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                ...
                <div className="flex gap-4 mt-4 md:mt-0">
                    {!isAdminView ? (
                        <>
                            <button onClick={() => {}} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2"><Truck size={20} /> Demander une Livraison</button>
                            <button onClick={() => {}} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2"><PlusCircle size={20} /> Nouvelle Vente</button>
                        </>
                    ) : (
