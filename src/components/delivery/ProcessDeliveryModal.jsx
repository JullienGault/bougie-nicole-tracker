...
const ProcessDeliveryModal = ({ request, onClose, onCancelRequest }) => {
    ...
    const handleSaveChanges = async (reason) => {
        ...
        try {
            ...
            await updateDoc(requestDocRef, {
                items: editableItems, modificationReason: reason,
                originalItems: request.originalItems || request.items
            });
    ...
    const handleAdvanceStatus = async () => {
        ...
        try {
            ...
            await addDoc(collection(db, 'notifications'), {
                recipientUid: request.posId, message: `Le statut de votre commande est maintenant : "${DELIVERY_STATUS_STEPS[nextStatus]}".`,
                createdAt: serverTimestamp(), isRead: false, type: 'DELIVERY_UPDATE'
            });
            onClose();
        } catch (error) {
